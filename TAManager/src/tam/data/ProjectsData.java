...
public class ProjectsData {
    ...
    ObservableList<ProjectTeamItems> projectsTeamTable;
    ObservableList<ProjectStudentItems> projectsStudentTable;
    ...
    public ProjectsData(TAManagerApp initApp) {
        // KEEP THIS FOR LATER
        app = initApp;

        // CONSTRUCT THE LIST OF TAs FOR THE TABLE
        projectsTeamTable = FXCollections.observableArrayList();
        projectsStudentTable = FXCollections.observableArrayList();
        projectsTeamExport = FXCollections.observableArrayList();
        projectsDataExport = FXCollections.observableArrayList();
    ...
    public void resetData() {
        projectsTeamTable.clear();
        projectsStudentTable.clear();
    }
    ...
    public ObservableList getProjectsTeamTable() {
        return projectsTeamTable;
    }

    public ObservableList getProjectsStudentTable() {
        return projectsStudentTable;
    }
    ...
    public void addTeam(String initTeamName, String initTeamColor, String initTeamTextColor, String initTeamLink, boolean flag) {

        if (flag) {
            ProjectTeamItems projectsTeam = new ProjectTeamItems(initTeamName, initTeamColor, initTeamTextColor, initTeamLink);
            projectsTeamTable.add(projectsTeam);
        }
        String colorHex = initTeamColor.replaceAll("#", "");
        String color2Hex = initTeamTextColor.replaceAll("#", "");
        ...
        ProjectTeamExport teamExport = new ProjectTeamExport(initTeamName, Integer.toString(r), Integer.toString(g), Integer.toString(b), textColor);
        projectsTeamExport.add(teamExport);

    }

    public void addStudents(String initFirstName, String initLastName, String initTeam, String initRole, Boolean flag, Boolean flag2) {
        CSGWorkspace workspace = (CSGWorkspace) app.getWorkspaceComponent();
        ProjectStudentItems projectsStudent = null;
        if (flag2) {
            projectsStudent = new ProjectStudentItems(initFirstName, initLastName, initTeam, initRole);

            projectsStudentTable.add(projectsStudent);
        }
        if(!flag2){
            projectsStudentTable.clear();
        }
        if (flag) {
            String link = "";
            for (int i = 0; i < projectsTeamTable.size(); i++) {
                for (int j = 0; j < projectsStudentTable.size(); j++) {
                    if (projectsStudentTable.get(j).getTeam().equalsIgnoreCase(projectsTeamTable.get(i).getTeamName())) {
                        students.add(projectsStudentTable.get(j).getFirstName() + " " + projectsStudentTable.get(j).getLastName());
                        link = projectsTeamTable.get(i).getTeamLink();
                    }
                }

                ProjectsDataExport dataExport = new ProjectsDataExport(projectsTeamTable.get(i).getTeamName(), students, link);
                projectsDataExport.add(dataExport);
                students = new ArrayList<>();
            }
            workspace.getProjectWorkspace().reloadWorkspace();
        }
    }
